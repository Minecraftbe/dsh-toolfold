...
> DSH Web GUI 插件，提供**类 Codex 的工具折叠体验**：把连续的**工具调用**和**已完成的思考**
> 折叠成一条简洁的折叠条，只展示最后一个调用的一行摘要，点击即可展开/收起。
...
- **连续工具调用折叠为一条**：相邻的工具调用（中间只隔着已完成的思考也算同一段）整段收起，
  条内直接显示**最后一个调用**的一行摘要，并标注「已折叠 N 个工具调用 · 点击展开」；
- **思考随组折叠**：已完成的思考不会隔开工具调用，默认随组隐藏；开启「保留思考」后，
  收起时隐藏、展开时按**原顺序插回调用之间**，内容不会消失；
- **进行中的思考独立显示**：流式思考保持原样可见，完成后自动并入组内收起；
...
| **保留思考** | 开启后已完成的思考随工具调用一起折叠，展开时按原顺序显示在调用之间；关闭则思考直接隐藏 |
...
- **进行中的思考**与 **AI 文字输出**会隔开工具调用（进行中的思考完成后自动并入组内）；
