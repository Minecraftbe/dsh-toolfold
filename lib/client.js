 ...
 *     emptied row is removed from layout so no gap remains). If the row was
 *     emptied only because the answer text had not started yet, the next
 *     content mutation inside it un-empties it, so the streamed answer is
 *     never trapped inside a display:none row. With "保留思考" on they stay
 *     visible: between the folded bars in split mode, or folded with the
 *     run and re-inserted in their original order between the calls on
 *     expand in merge mode.
 ...
 *   - enabled: master switch (default on). Off fully unloads the engine —
 *     every fold, bar, injected style and class is removed and the chat
 *     returns to the product's default view; the settings card stays
 *     available so it can be flipped back on at any time;
 *   - durMs: expand/collapse animation duration (0–2000, default 240);
 *   - thinkAuto: thinking follows the product's official completed-Turn fold
 *     (「对话显示 = Compact」): when that fold is active, settled thinking is
 *     preserved (the big block is already collapsed, so thinking should not
 *     silently vanish); when it is not, settled thinking is hidden by
 *     default to save space (default on);
 *   - keepThink: settled think stays visible instead of hidden when 自动跟随
 *     is OFF (default off);
            ...
            yield slots.register(
              {
                ...
                inject: function () {
                  return {
                    ...
                    setThinkAuto: function (on) { bridge.write('thinkAuto', on); },
