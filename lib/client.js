 ...
 *
 *  1. Runs of `tool-call` rows fold into ONE compact bar: all cards of the
 *     run hide and the bar shows the LAST call's own one-line summary
 *     (cloned from the product card) plus "已折叠 N 个工具调用 · 点击展开".
 *     Clicking the bar toggles the run open/closed (state keyed by the run's
 *     first call). Completed thinking between calls does NOT break a run —
 *     a step that only settled Think rows occupy is transparent to the
 *     merge, so [read][thinking][write] folds the same way [read][write]
 *     does. Any visible non-tool content (AI text output, in-progress
 *     thinking, user messages) still ends a run.
 *  2. Settled Think rows (`[data-variant="think"][data-state="ok"]`, the
 *     product's own disclosure marker) are hidden entirely by default. With
 *     "保留思考" on they fold WITH the tool-call run instead: hidden while
 *     the run is collapsed, shown in their original order between the calls
 *     when it is expanded — nothing disappears. An assistant row left empty
 *     by a hidden think is removed from layout so no gap remains.
 ...
 *     bounce); collapsing cascades them back up (staggered spring rise — a
 *     small anticipatory dip, then they accelerate away) while each row's
 *     HEIGHT simultaneously shrinks to zero, so the content below the
 *     run follows upward continuously — no blank hole and no end snap. With
 *     "保留思考" on, settled think rows join the cascade in their original
 *     position between the calls. When the reader is pinned near the
 *     bottom, expanding keeps the bar's viewport position fixed (the
 *     product's sticky-bottom follow would otherwise shove the bar out of
 *     the top).
 *
 ...
 *   - keepThink: settled think folds with the run and reappears on expand
 *     (default off);
...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var DEFAULT_SETTINGS = { durMs: 240, keepThink: false, stats: false };

    function loadSettings() {
      var base = { durMs: DEFAULT_SETTINGS.durMs, keepThink: DEFAULT_SETTINGS.keepThink };
    ...
    var css = [
      ...
      '[data-chat-flow] [data-chat-flow-kind].ccxCollapsing{animation:ccxRise var(--ccx-dur,240ms) cubic-bezier(0.36,-0.56,0.66,0) both;animation-delay:calc(var(--ccx-i,0)*var(--ccx-step,45ms))}',
    ...
    function installCollapseEngine(doc, settings, timers) {
       ...
       *    starts hiding, empties its assistant row, and stops separating
       *    runs) — the merge must be rebuilt;
       ...
       * the row's think blocks (count + data-states + keepThink flag).
       * Reassessed only when the signature changes; the check is purely
       * structural, so steady-state passes perform zero layout reads.
       *
       * A row that only settled Think rows occupy is transparent to run
       * building — completed thinking folds WITH the tool calls instead of
       * separating them — and empty (hidden) when think is not preserved.
       * With "保留思考" on the row folds with the run and reappears in its
       * original position on expand.
       */
      var rowInfo = new WeakMap();
      var NEUTRAL_ASSESSMENT = { sig: '', thinks: [], transparent: false, empty: false };

      function thinkSig(thinks, keepThink) {
        var sig = keepThink ? '1' : '0';
      ...
      function assessRow(row, keepThink) {
        ...
        if (thinks.length > 0 && thinkOnlyContent(row)) {
          ...
          if (allOk) {
            // A row made of settled think alone never separates a run: it
            // folds with it, and is hidden unless 保留思考 keeps it for the
            // expanded view.
            assessment.transparent = true;
      ...
      function pinBar(bar, run) {
        ...
        var dur = durMs();
        var step = stepMs();
        var until = Date.now() + run.rows.length * step + dur + 260;
        var stopped = false;
        var cancel = function () {
          stopped = true;
      ...
      function pass(flow) {
        ...
        // (settled-thinking-only) rows between them. Any other row — AI text,
        // in-progress thinking, user/steering/command content — ends a run.
        // sawTool/sawThink feed the light→full observer upgrade: a flow with
        // foldable content needs subtree + data-state coverage.
        var runs = [];
        var current = [];
        var sawTool = false;
        var sawThink = false;
        for (var i = 0; i < rows.length; i++) {
          ...
          if (kind === 'tool-call') {
            ...
            var assessment = assessRow(row, keepThink);
        ...
        for (var r = 0; r < runs.length; r++) {
          ...
          // for the whole group and displays the last call itself. With
          // "保留思考" on the run's settled-think rows fold with it too:
          // hidden while collapsed, shown in their original order between
          // the calls when expanded (they ride the same cascade). A run
          // mid collapse animation keeps its rows visible for the rise.
          ...
          mergedRuns.push({
            ...
            toolRows: toolRows,
            thinkRows: thinkRows,
            // The run's rows in DOM order: cards and settled-think rows
            // interleaved exactly as they appear, used by the waterfall.
            rows: run,
        ...
        for (var x = 0; x < rows.length; x++) {
          ...
          var assessment = assessRow(candidate, keepThink);
    ...
    function SettingsCard(props) {
      ...
      var thinkField = React.createElement('div', { className: 'ccxField' },
        ...
        React.createElement('p', { className: 'ccxFieldHint' }, '思考随工具调用一起折叠：收起时隐藏，展开时按原顺序显示在调用之间'));
      ...
      var body = React.createElement('div', { className: 'ccxBody' },
        durField, thinkField, statsField,
